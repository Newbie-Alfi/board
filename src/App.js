...
export function App() {
  ...
  useEffect(() => {
    ...
    listAnchorNodes.map((anchorNode, i) => {
      if (anchorNode.current !== null)
        setAnchorParametrs((prev) => [
          ...prev,
          {
            Y:
              anchorNode.current.getBoundingClientRect().top +
              window.pageYOffset,
  ...
  let updateCriteria = useCallback((value) => setValue(value), [value]);
      ...
      >
        <div className="App">
          <Route exact path="/">
            ...
            <MainStudents />
            <Separator text={"Выбирите факультет"} />
